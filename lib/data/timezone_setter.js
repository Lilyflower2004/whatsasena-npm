...
async function time(number) {
  ...
  if (number.startsWith('90')) { // Turkey
    ...
    data = new Date().toLocaleString('ES', { timeZone: 'America/Noronha' }).split(' ')[1]
  } else if (number.startsWith('51')) { // Perú
    data = new Date().toLocaleString('ES', { timeZone: 'Europe/Madrid' }).split(' ')[1]
  } else if (number.startsWith('34')) { // España
    data = new Date().toLocaleString('GB', { timeZone: 'Europe/Madrid' }).split(' ')[1]
