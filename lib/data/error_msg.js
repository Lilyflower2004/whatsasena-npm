...
async function err_msg(Language) {
  ...
  if (Language == 'TR') { 
    ...
    data = '*-- REPORTE DE ERROR [SKUELETOR] --* \n*Skueletor tuvo un error!*\n_Este registro de errores puede contener su número o el número de una contraparte. Por favor, tenga cuidado con eso!_\n_Puede escribirme por privado mediante Telegram para informar sobre este error. t.me/DKzippO_\n_Este mensaje debería haber ido a su número (mensajes guardados).\n\n*Error que ocurre:* ```' + '{real_error}' + '```'
  } else if (Language == 'SI') {
